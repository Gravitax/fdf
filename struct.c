...
/*   Updated: 2019/07/30 15:09:55 by maboye           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		copyin_struct(t_fdf *data, char **stock, int i)
{
	...
	* (ft_tablen((void **)stock)))))
		return (0);
	data->min_z = i == 0 ? ft_atoi(stock[0]) : data->min_z;
	data->max_z = i == 0 ? ft_atoi(stock[0]) : data->max_z;
	j = -1;
	while (stock && stock[++j])
	{
		...
		data->map[i][j].za = ft_atoi(stock[j]);
		data->map[i][j].zsave = ft_atoi(stock[j]);
		data->map[i][j].hexa = NULL;
		if (!stock_hexa(data, stock[j], i, j))
			return (0);
		data->min_z = ft_atoi(stock[j]) < data->min_z ?
			ft_atoi(stock[j]) : data->min_z;
		data->max_z = ft_atoi(stock[j]) > data->max_z ?
			ft_atoi(stock[j]) : data->max_z;
	}
	return (1);
}
